...
**（一）当仅有一对 if else 语句，可以使用 return 来替代 else**
...
**注意：当字符串中包含该字段，则返回它在该字符串中首次出现的位置（即索引值），否则返回 -1，常作为判断条件，如检索某个字段是否在白名单内，当大于（或者不等于） -1 时，表示存在**
...
2. **ondragover：被拖拽的元素在目标容器范围内移动时触发**
...
**尤其注意：该方法没有 return，且终止循环不能使用 break，如果想要终止循环，用 try catch 包裹循环体，然后根据条件手动抛出异常即可终止循环**

**尤其注意：forEach 可以修改原数组，可以对遍历的每一项 item 进行修改，当 item 为对象时，通常会为其增加一个属性（item.属性名 = 属性值)**
...
**方式五（主要用于修改原数组的某几项，并返回一个新数组）：var 变量名 = 数组名 . map（function（item，index）{ return 新数据 }）**

**注意：此处 item 表示迭代（遍历）过程中当前的每一个元素，此处 index 表示当前每一个元素的索引值**

**注意：该方法会遍历数组的每一项，必须写 return 返回一个新数据（该数据是由当前项根据条件进行修改得到），或者 return 原始项（item），遍历过程中，无论 return 的是原始项还是修改后的新数据，都会替代当前遍历到的那一项，追加到新数组中，用一个变量接收**

**尤其注意：return 是多次的，是针对当前遍历的每一项进行返回的，并不是只 return 一次，这里的 return 相当于 push**



#### ES6 新增的数组扩展方法

**方法一（找到符合条件的第一项，并返回这个成员本身）：var 变量名 = 数组名 . find（function（item）{ return 指定条件 }）**

**注意：此处 item 表示迭代（遍历）过程中当前的每一个元素**

**注意：该方法必须写 return 来作为指定条件，当找到符合条件的第一项，就返回这个成员本身，后面不再遍历，否则就返回 undefined **

**尤其注意：如果是数组嵌套对象的形式，使用 find 方法，会找到的符合条件的那个对象，但返回的其实是该对象的地址，对该项进行修改，可以影响原始数据**
...
**方法三（检测数组或者字符串中是否包含某个值）：var 变量名 = 数组名或字符串名 . includes（被检测的值）**

**注意：包含该值返回 true，否则返回 false**

**尤其注意：该方法同时适用于字符串和数组，两者皆可使用**



**方法四（reduce 迭代器）：var 变量名 = 数组名 . reduce（function（val，item，index）{ return 返回的值 }，初始值）**

**注意：此处 item 表示迭代（遍历）过程中当前的每一个元素，此处 index 表示当前每一个元素的索引值，大部分方法都有这两个参数，但 reduce 的第一个参数为 val，它类似于外部定义的一个变量，有初始值，则为初始值，无初始值，则为数组的第一项，然后从第二项开始循环，而 return 返回的值会作为下一次循环的 val 的初始值，变量名接收的是最终的 val 值**



#### Lodash 常用方法

**（一）筛选对象：var 变量名 = _.pickBy（对象名 , function（value，key）{ return 筛选条件 }）**

**注意：如果不指定筛选函数，var 变量名 = _.pickBy（对象名），默认会保留所有 "真值"（排除 `false`、`null`、`undefined`、`0`、`''` 等）**



**（二）去除对象某个属性：var 变量名 = _.omit (对象，[ '属性1', '属性2' ])**
...
1. **let 相关知识**
   ...
   **let 具有块级作用域，即：只能在大括号范围内访问使用 let 声明的变量，因此可以代替立即执行函数，形成私有变量，在每次循环后都能访问对应 i 值**

   **let 不会提升变量，也可以防止 for 循环造成的变量污染（var 声明的变量无法自行销毁，存在于全局中）**

   **暂时性死区：在变量未声明前无法访问到该变量，这段空期被称为暂时性死区，同时，花括号中有 let，那么就封闭死整个花括号区域，只能访问花括号内部的变量，哪怕外界有同名变量，也无法访问（不具备作用域链）**
...
**注意：如果一个变量保存的是引用类型数据，用 const 声明也可以，因为正常情况只是更改引用类型的内部数据，不会对变量整体进行赋值**
...
2. **对象解构**
   ...
   **深入理解：对象解构的本质就是将对象的下一级放在花括号内，解构的目标就是对象本身（ Obj.xxx => let { xxx } = Obj ）**

   **尤其注意：对象解构出的属性名，必须和对象名中的属性名保持一致（切记），不能改变，如果需要改变，可以使用冒号进行重命名**

   **尤其注意：如果是函数的某个参数（无论是形参还是实参）是对象，同样可以使用对象解构作为函数的参数使用**

3. **对象的深层解构（多级嵌套）**

**已知：对象名.xxx.ooo，如果想解构出 ooo，可采取如下两种方式**
...
**语法：const 函数名 = （ 形参1，形参2 ） => { 函数处理程序 ；看情况 return 返回值，可无返回值 }**

**尤其注意：当形参个数只有一个，可以省略括号，如果花括号中只有一句，且该句包含 return，可省略花括号，并省略 return**

```javascript
const 函数名 = 形参 => 返回值
```

**尤其注意：箭头函数没有自己的 this 指向，this 指向的是它的上一层（父级）**

**极其重要：通常作为事件函数，不确定是否需要使用 this 关键字，建议使用普通函数，而在其它地方，确定不会用到 this 关键字，则使用箭头函数**

**尤其注意：当一个箭头函数返回的直接是一个对象形式，可写成如下形式，多见于 pinia 中的 state**
...
// pinia 中的 state 场景，该写法是 “属性：函数” 的简写形式
state(){
    return{
        变量名：值
    }
}
// pinia 中的 state 场景，还原成 “属性：箭头函数” 的原始形式，且箭头函数返回的是一个对象
state: () => ({
    变量名：值
}),
```
...
##### （五）剩余参数（Rest 参数）和扩展运算符

1. **剩余参数又称 Rest 参数，作为函数的一个特殊参数而存在：const 函数名 = （ ...args ） => { 函数处理程序 }**

   **注意：当调用函数时，传递了多个参数，...args 可以拿到所有的参数，且 args 为数组形式（真正的数组）**

   **args 与 arguments 的区别：前者是真正的数组，arguments 虽然能拿到所有参数，却是伪数组，需要通过 Array.from（arguments）转换成真正的数组**

2. **扩展运算符，可以将对象或数组中的每一项依次取出：...对象名或数组名**
