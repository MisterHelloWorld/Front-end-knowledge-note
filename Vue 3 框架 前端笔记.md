...
##### （十二）进阶：hooks 抽离封装（以 ts 为例，需先了解 ts）

1. **在 src 下新建 composables 文件夹：此处存放非通用的 hooks 方法，适用于单独某个功能的逻辑封装，每个方法单独的文件，命名就是 useXXX.ts**

**此处以 useLogin.ts 进行举例，新建此文件，写入以下代码：**
...
export const useLogin = () => {
  ...
  const rules = reactive<FormRules>({
    username: [
      ...
      { min: 2, max: 6, message: '用户名长度在2至6之间', trigger: 'blur' }
    ],
    password: [
      { required: true, message: '请输入密码', trigger: 'blur' },
      { min: 6, max: 12, message: '密码长度在6至12之间', trigger: 'blur' }
    ]
  })
  ...
  const loginBtn = async () => {
    // 判断form实例是否存在,一般情况都校验表单了肯定存在,这里是防止表单dom晚于登录按钮渲染,提前触发的情况
    // 因此做一个严谨的判断
    if (!formRef.value) return
    // 整体校验
    await formRef.value.validate(async (isOk: boolean) => {
      if (isOk) {
        // 登录逻辑
...
const { loginBtn, formRef, rules, form } = useLogin()
// 绑定表单操作...
```



2. **在 src\utils 下新建 hooks.ts 文件，此处存放通用的 hooks 方法（详情可见十三：数据懒加载）**
...
**解释：类型是对值的声明，代表这个变量保存的内容是什么类型**

**注意：类型声明都是小写形式（如：number 或 string 或 boolean）**
...
**注意（按照习惯，一般不声明）：简单类型会根据具体的值进行类型推断，一般不需要特别声明**
...
```typescript
// 数组类型的第一种写法：number型数组
let 变量名: number[] = [1, 2, 3]
// 数组类型的第二种写法：string型数组（按照习惯，一般采用此方式，此处用到了泛型）
...
```typescript
let 变量名：number | string | boolean = 值
```

**如果数组中有多个类型（Number、String、Boolean），同样使用联合类型（即：使用 | 将多个类型组合在一起）**

```typescript
// 此处以数组泛型举例
let 变量名: Array<number | string | boolean> = [1, 'a', true]
```
...
**当类型比较复杂时，可以使用 type 关键字，创建一个类型别名单独保存这个类型**

1. **定义类型别名：**

```typescript
type 类型别名 = Array<number | string | boolean>
```
...
**简单记忆：无论是普通函数还是箭头函数，声明类型都是给参数和返回值进行声明，且一定是连在一起的：`(参数1: 类型, 参数2: 类型): 返回类型`**
...
// 返回类型若是 Promise 对象
const 函数名 = (参数：类型):Promise<unknown> => {
  return Promise 对象
}
```
...
**（重点）箭头函数配合类型别名（类型别名只能配合箭头函数使用，普通函数不能使用类型别名）：**
...
// 特殊对象，键值对内容未知
let 对象名: Record<string, any> 或者 {[key: string]: any} = {
  未知键：未知值
}
```
...
**新特新：在 vue3.3+ 版本之后，可以直接在 script 标签内，通过以下语法直接配置 name：**
...
**前提：先确定是否有该依赖相关类型声明文件可以进行下载，如果没有再自己进行声明**

1. **确定插件依赖名称（此处以 id-validator 举例）**
2. **在 types 文件夹下，新建类型声明文件（id-validator.d.ts）**
3. **按照该依赖的使用方法，大概推断出怎么使用的，进行声明**

```typescript
// 比如该依赖使用方式是 const idValidator = new IDValidator()，可以推断出是一个类
// 然后其中有两个方法，一个是校验身份证是否有效，一个是返回相关信息
...
1. **进阶封装方法：在 src/utils 目录下新建 request.ts 文件中，除了基础封装（请求拦截，响应拦截），继续定义一个请求工具函数，最后导出该工具函数即可**
...
2. **在 src 下的 types 文件夹中，新建多个类型声明文件（如 user.d.ts）**

```typescript
// 请求参数类型（最好以 Params 结尾命名，方便区分）
...
// 请求结果类型（result 部分，也就是泛型 T 部分，最好以 Result 结尾命名，方便区分）
...
3. **定义请求接口：新建多个请求文件（如 user.ts)**

```typescript
import type { 请求参数类型，请求结果类型 } from '@/types/consult'

import request from '@/utils/request'

// 请求接口
export const 请求接口名称 = (params: 请求参数类型) =>
  request<请求结果类型>('/home/page/doc', 'GET', params)
```
...
**在 utils 中新建 md5.js 文件，书写如下代码，在需要加密的地方调用该方法即可：**

```javascript
// md5 加密方法：传递参数（需要加密的字符串,加密位数）
const md5 = function (str, bit) {
  var sMessage = str;
