...
1. **给定一个最外层容器（固定宽高，设置超出部分隐藏），里面再加一个不限高度的容器 A，该容器是滚动的整体容器， A 容器内部是每一个列表项 B**
...
**通用方法（实际开发中使用该方式）：以上步骤从第二点开始整个流程，是无缝滚动的实现原理，需要计算出需要补充的列表项数量和总计列表项数量的高度，略显麻烦，实际开发中，直接补充相同数量的列表项在末尾，然后向上移动百分之百即可**
