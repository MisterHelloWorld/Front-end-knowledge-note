...
const length = getLength(几何信息)
// 根据线要素的几何信息，获取中间任意位置的经纬度（0-1，一般0.5为中间点）
const 中间点 = 几何信息.getCoordinateAt(0.5)
    ...
    创建图层的方法 (数据列表) {
      ...
      数据列表.forEach((item) => {
        ...
        const feature = new Feature({
          ...
          key: '要素唯一标识',
          // ...其它需要携带的信息数据（为了方便起见，一般直接把当前对象数据全部保留）
          data: item
        })
        // 给要素指定 id 唯一标识（通过 “某个图层源.getFeatureById(id 唯一标识)” 查找要素，正常情况很少使用，点击事件里面能拿到当前点击的要素）
      ...
      const 图层名称 = new VectorLayer({
        ...
        style: (feature) => {
          ...
          // 性能优化：若是固定不变的样式，可单独抽离 nwe Style()
          return [
            // 多个样式相互独立
            new Style({
              // ...与下方同理     
            ...
            new Style({
              ...
              image: new Icon({
                ...
                rotation: 旋转角度,
                scale: 缩放大小,
                // 图标按照经纬度偏移位置（默认按比例 fraction）
                // 此处以圆形图标居中举例，若是底部是尖尖图标，则换成 [0.5, 1]
                anchor: [0.5, 0.5],
                // 若需要做偏移，可单独指定是否按比例 fraction 还是像素 pixels
                anchorXUnits: 'fraction',
                anchorYUnits: 'pixels',
                // 旋转角度（弧度：利用以下 Math.PI 公式，将数字形式的度数转换为弧度，正右方向逆时针旋转角度递增，且为负数）
                rotation: -数字度数 * (Math.PI / 180)
              }),
              // 点要素：image 属性（直接绘制圆）
              ...
              text: new Text({
                ...
                text: `此处是需要显示的文本\n用来换行`,
                // 文字大小和字体（可使用本地字体）
                font: 'blod 14px Calibri,sans-serif',
                // 文字颜色
                fill: new Fill({
                  color: '#ffffff'
                }),
...
```javascript
    // 使用方式：只需要维护 activeId 和 activeType 这两个变量，最后调用该方法即可
    highlightFeature () {
      if (!this.图层名称) return
      // 拿到当前图层所有要素
      const features = this.图层名称.getSource().getFeatures()
      // 遍历所有要素
      features.forEach((feature) => {
        // 拿到所有要素信息（前提：在创建图层时，每个要素携带了当前对象数据 data）
        const { data } = feature.getProperties()
        // 判断当前要素是否需要高亮（根据点击 ID 或当前需要高亮的图标类型）
        const isSelected = data.id === this.activeId || this.activeType.includes(data.图标类型)
        // 根据类型设置不同的样式（固定样式可以提前抽离，如：const 高亮样式1 = new Style({......}）
        if (data.图标类型 === 1) {
          feature.setStyle([isSelected ? 高亮样式1 : 默认样式1])
        } else if (data.图标类型 === 2) {
          feature.setStyle([isSelected ? 高亮样式2 : 默认样式2])
        } else if (data.图标类型 === 3) {
          feature.setStyle([isSelected ? 高亮样式3 : 默认样式3])
        }
      })
    },
```
